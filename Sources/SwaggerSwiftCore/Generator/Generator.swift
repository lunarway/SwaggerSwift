...
public struct Generator {
  ...
  public func parse(
    swaggerFilePath: String?, githubToken: String, verbose: Bool = false, dummyMode: Bool = false,
  ...
  private func download(
    githubToken: String, organisation: String, serviceName: String, branch: String,
    swaggerPath: String, urlSession: URLSession
  ) async throws -> Data {
    let url = URL(
      string:
  ...
  private func write(
    ...
    if !dummyMode {
      ...
      // create directories
      try fileManager.createDirectory(
        atPath: modelDirectory, withIntermediateDirectories: true, attributes: nil)
    }
  ...
  private func createCommonLibrary(
    path: String, commonLibraryName: String, swaggerFile: SwaggerFile,
    accessControl: APIAccessControl, globalHeadersModel: GlobalHeadersModel,
    fileManager: FileManager
  ) throws {
    ...
    if swaggerFile.globalHeaders.count > 0 {
      ...
      let globalHeadersFileContents = globalHeadersModel.toSwift(
        swaggerFile: swaggerFile, accessControl: accessControl)
  ...
  private func createPackageSwiftFile(
    at path: String, named name: String, commonLibraryName: String, apis: [String],
