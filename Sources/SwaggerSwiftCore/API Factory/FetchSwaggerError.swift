enum FetchSwaggerError: Error {
    ...
    func logError() {
        ...
        case .requestFailed(let serviceName, let branch, let statusCode):
            ...
            if branch != "master" {
                log("[\(serviceName)]: ⚠️⚠️⚠️ The branch was defined as ´\(branch)´. Perhaps this branch is deleted now? ⚠️⚠️⚠️", error: true)
            }

            log("[\(serviceName)]: - If this is happening to all of your services your github token might not be valid", error: true)
