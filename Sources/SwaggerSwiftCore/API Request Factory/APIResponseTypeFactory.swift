...
public struct APIResponseTypeFactory {
    ...
    func make(forResponses responses: [Response], forHTTPMethod httpMethod: HTTPMethod, at servicePath: String, swagger: Swagger) -> [APIRequestResponseType] {
        let errorResponses = responses.filter { !$0.statusCode.isSuccess }
        let successResponses = responses.filter { $0.statusCode.isSuccess }

        let responsesTypes = responses
            .sorted(by: { $0.statusCode.rawValue < $1.statusCode.rawValue })
            .compactMap { response -> APIRequestResponseType? in
                let statusCode = response.statusCode
                let isSuccessResponse = response.statusCode.isSuccess ? successResponses.count > 1 : errorResponses.count > 1
                ...
                case .array(let type):
                    if case .object(let typeName) = type {
                        return APIRequestResponseType.array(response.statusCode,
                                                            response.statusCode.isSuccess ? successResponses.count > 1 : errorResponses.count > 1,
                                                            typeName: typeName)
                    } else {
                        log("[\(swagger.serviceName) - \(httpMethod.rawValue) \(servicePath)] Unsupported type '\(type)' in array]")
                        return nil
                    }
