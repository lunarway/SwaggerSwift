...
extension APIRequest {
  ...
  private func makeRequestFunction(
    serviceName: String?, swaggerFile: SwaggerFile, accessControl: String
  ) -> String {
    ...
    let responseTypes = self.responseTypes
      .map {
        $0.print(
          apiName: serviceName ?? "", errorType: returnType.failureType.toString(required: true))
      }
      .joined(separator: "\n")
  ...
  func toSwift(
    serviceName: String?, swaggerFile: SwaggerFile, embedded: Bool, accessControl: String,
