...
public struct APIRequestFactory {
  ...
  public enum APIRequestFactoryError: Swift.Error {
    case unsupportedMimeType(
      serviceName: String, httpMethod: String, servicePath: String, mimeType: String)
    case missingConsumeType(serviceName: String, httpMethod: String, servicePath: String)
  }

  func generateRequest(
    for operation: SwaggerSwiftML.Operation, httpMethod: HTTPMethod, servicePath: String,
    swagger: Swagger, swaggerFile: SwaggerFile, pathParameters: [Parameter]
    ...
    let responses: [Response] = try operation.responses.compactMap {
      ...
      if let (responseType, embeddedDefinitions) = try parse(
  ...
  private func resolveQueries(parameters: [Parameter], swagger: Swagger) throws -> [QueryElement] {
    ...
    return try queryParameters.map {
      try queryParameterToQueryElement(parameter: $0, swagger: swagger)
    }
  }

  private func queryParameterToQueryElement(parameter: Parameter, swagger: Swagger) throws
    -> QueryElement
  {
    ...
    case .query(let type, _):
      ...
      case .string(let format, let enumValues, _, _, _):
        ...
        if let format = format {
          switch format {
          case .int32, .long, .float, .double, .string, .byte, .binary, .boolean, .password, .email,
            .unsupported:
            return QueryElement(
      ...
      case .number, .integer, .boolean, .file:
        return QueryElement(
          fieldName: parameter.name,
          fieldValue: parameter.name.camelized,
          isOptional: parameter.required == false,
  ...
  private func consumeMimeType(
    forOperation operation: SwaggerSwiftML.Operation, swagger: Swagger, httpMethod: String,
    servicePath: String
  ) throws -> APIRequestConsumes {
    if let rawConsume = operation.consumes?.first ?? swagger.consumes?.first {
      if let consume = APIRequestConsumes(rawValue: rawConsume) {
        return consume
      } else {
        log(
          "[\(swagger.serviceName) \(httpMethod) \(servicePath)] ⚠️ SwaggerSwift does not support consume mime type '\(rawConsume)'"
        )
        throw APIRequestFactoryError.unsupportedMimeType(
          serviceName: swagger.serviceName, httpMethod: httpMethod, servicePath: servicePath,
          mimeType: rawConsume)
      }
    } else {
  ...
  private func resolveFunctionName(httpMethod: String, servicePath: String, operationId: String?)
    -> String
  {
