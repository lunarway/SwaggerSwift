...
public struct RequestParameterFactory {
  ...
  func make(
    forOperation operation: SwaggerSwiftML.Operation, functionName: String,
    responseTypes: [ResponseTypeMap], pathParameters: [Parameter], swagger: Swagger,
    ...
    let (formDataParameters, formDataModels) = try resolveFormDataParameters(
      parameters: parameters, typePrefix: typeName, swagger: swagger)
  ...
  private func createResultEnumType(
    types: [ResponseTypeMap], failure: Bool, functionName: String, swagger: Swagger
    ...
    if filteredTypes.count > 1 {
      let typeName =
        functionName.components(separatedBy: "_").map { $0.capitalizingFirstLetter() }.joined(
          separator: "") + (failure ? "Error" : "Success")
  ...
  private func resolveInputHeadersToApi(
    parameters: [SwaggerSwiftML.Parameter], typePrefix: String, isInternalOnly: Bool,
    swagger: Swagger, swaggerFile: SwaggerFile
    ...
    let globalHeaderFields =
      ...
      .map {
        ModelField(
          description: nil, type: .string(defaultValue: nil), name: convertApiHeader($0.name),
          isRequired: false)
      }
  ...
  private func resolvePathParameters(
    parameters: [SwaggerSwiftML.Parameter], typePrefix: String, swagger: Swagger
  ) throws -> ([FunctionParameter], [ModelDefinition]) {
  ...
  private func resolveBodyParameters(
    parameters: [SwaggerSwiftML.Parameter], typePrefix: String, namespace: String, swagger: Swagger
  ) throws -> (FunctionParameter, [ModelDefinition])? {
