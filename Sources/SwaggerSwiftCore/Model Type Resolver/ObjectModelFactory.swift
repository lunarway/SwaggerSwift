...
public class ObjectModelFactory {
    ...
    func make(properties: [String: Node<Schema>], requiredProperties: [String], allOf: [Node<Schema>]?, swagger: Swagger, typeNamePrefix: String, schema: Schema, namespace: String, customFields: [String: String]) throws -> (TypeType, [ModelDefinition]) {
        ...
        if let allOf = allOf, allOf.count > 0 {
            let allOfParts: [AllOfPart] = try parseAllOf(allOf: allOf,
                                                         typeName: typeName,
                                                         namespace: namespace,
                                                         swagger: swagger)
            ...
            let (resolvedProperties, inlineModels) = try resolveProperties(properties: properties,
    ...
    private func resolveProperties(properties: [String: Node<Schema>], withRequiredProperties requiredProperties: [String], namespace: String, swagger: Swagger) throws -> ([ModelField], [ModelDefinition]) {
        var totalFields = [ModelField]()
        var totalModels = [ModelDefinition]()
        
        for (name, schemaNode) in properties {
            if let (field, models) = try resolveProperty(named: name,
                                                         namespace: namespace,
    ...
    private func resolveProperty(named: String, namespace: String, schemaNode: Node<Schema>, withRequiredProperties requiredProperties: [String], swagger: Swagger) throws -> (ModelField, [ModelDefinition])? {
    ...
    private func parseAllOf(allOf: [Node<Schema>], typeName: String, namespace: String, swagger: Swagger) throws -> [AllOfPart] {
        ...
        for allOf in allOf {
            ...
            case .reference(let reference):
                ...
                guard case SchemaType.object = schema.type else {
                    log("[\(typeName)] Non object type received as a part of an all of statement", error: true)
                    continue
                }
                
                let modelReference = try ModelReference(rawValue: reference)
                
                let part = AllOfPart(typeName: modelReference.typeName,
                                     fields: [],
            ...
            case .node(let schema):
                ...
                for property in properties {
                    if let (field, inlineModels) = try parseAllOfPartProperty(named: property.key,
                ...
                let allOfPart = AllOfPart(typeName: nil,
                                          fields: allFields,
    ...
    private func parseAllOfPartProperty(named name: String, propertySchema: Node<Schema>, requiredProperties: [String], namespace: String, swagger: Swagger) throws -> (ModelField, [ModelDefinition])? {
