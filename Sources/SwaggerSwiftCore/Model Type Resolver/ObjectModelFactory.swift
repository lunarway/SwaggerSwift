...
public class ObjectModelFactory {
    ...
    private func resolveProperties(
        properties: [String: Node<Schema>],
        withRequiredProperties requiredProperties: [String],
        namespace: String,
        swagger: Swagger
        ...
        for allOf in allOf {
            ...
            case .reference(let reference):
                ...
                guard case SchemaType.object = schema.type else {
                    log(
                        "[\(typeName)] Non object type received as a part of an all of statement",
                        error: true
                    )
                    continue
                }
