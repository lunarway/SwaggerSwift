...
public struct ModelTypeResolver {
  ...
  func resolve(
    forSchema schema: SwaggerSwiftML.Schema, typeNamePrefix: String, namespace: String,
    swagger: Swagger
    ...
    case .dictionary(let valueType, _):
      ...
      case .schema(let schema):
        ...
        return .init(
          .object(typeName: "[String: " + valueString + "]"), resolvedType.inlineModelDefinitions)
      }
    case .file:
      return .init(.void)
    case .freeform:
      return .init(
        ...
        [
          .array(
            .init(
              description: schema.description, typeName: typeNamePrefix,
  ...
  private func typeOfItems(
    schema: Schema, items: Node<Items>, typeNamePrefix: String, namespace: String, swagger: Swagger
    ...
    case .node(let node):
      ...
      case .number(
        let format, maximum: _, exclusiveMaximum: _, minimum: _, exclusiveMinimum: _, multipleOf: _):
      ...
      case .integer(
        format: _, maximum: _, exclusiveMaximum: _, minimum: _, exclusiveMinimum: _, multipleOf: _):
        return (.int(defaultValue: nil), [])
      case .boolean:
